...
const instanceTicket = axios.create({
    baseURL: import.meta.env.VITE_API_TICKET_URL,
    headers: {
        withCredentials: true,
    },
});

instanceTicket.interceptors.response.use((response) => {
        return response;
    }, async (error) => {
...
// If renewal is in progress, add the request to subscribers
            return new Promise((resolve) => {
                refreshSubscribers.push(() => {
                    resolve(instanceTicket(error.config));
                });
            });
        }
...
instanceTicket.interceptors.request.use(
    config => {
